...
# Pre-compiled regexes for performance (ChatGPT Fix #9) + Expanded Coverage + Additional Euphemisms
CRISIS_PATTERNS = [
    ...
    # ChatGPT Final Refinement: Additional ambiguous euphemisms
...
# ChatGPT Fix #8 - Locale-aware crisis resources (REFINED INTERNATIONAL FALLBACK)
CRISIS_RESOURCES = {
    "US": {
...
# Enhanced response validator patterns (ChatGPT Fix #6 + Refinements + Imperative Focus)
FORBIDDEN_RESPONSE_PATTERNS = [
    ...
    # ChatGPT Refinement: Focus on imperatives to avoid false positives
...
# Input validation patterns (mirror output validation for user input) + Refined Imperatives
FORBIDDEN_INPUT_PATTERNS = FORBIDDEN_RESPONSE_PATTERNS + [
    ...
    # Focus on first-person imperatives to avoid blocking observational questions
...
def log_potential_crisis_euphemisms(message):
    """Log potential crisis language that might have slipped through detection for review"""
    message_lower = message.lower()
    
    # Words that might indicate crisis but weren't caught by main patterns
    potential_indicators = [
        'stop', 'tired', 'give up', 'hopeless', 'helpless', 'trapped',
        'escape', 'way out', 'done', 'finished', 'over', 'pointless'
    ]
    
    # Look for concerning combinations
    concerning_combinations = [
        ('tired', 'life'), ('tired', 'everything'), ('give up', 'life'),
        ('no', 'hope'), ('no', 'point'), ('want', 'out'), ('need', 'escape'),
        ('done', 'everything'), ('over', 'life'), ('life', 'pointless')
    ]
    
    # Check for potential missed euphemisms
    found_indicators = []
    ...
    found_combinations = []
    for word1, word2 in concerning_combinations:
        if word1 in message_lower and word2 in message_lower:
            found_combinations.append(f"{word1}+{word2}")
    
    # Log if we find concerning patterns (in production, this would go to a logging service)
    if found_indicators or found_combinations:
        # In development, we can add to session state for review
        if 'potential_crisis_logs' not in st.session_state:
            st.session_state.potential_crisis_logs = []
        ...
        st.session_state.potential_crisis_logs.append(log_entry)
        
        # Keep only last 10 entries to prevent memory buildup
        st.session_state.potential_crisis_logs = st.session_state.potential_crisis_logs[-10:]
...
def has_explicit_crisis_language(message):
    ...
    # If no explicit crisis detected, log potential euphemisms for review
    if not has_crisis:
        log_potential_crisis_euphemisms(message)
...
def generate_identity_support_response(student_age, student_name=""):
    """Generate supportive, age-appropriate response for identity questions (ChatGPT Fix #5 + Balanced Approach)"""
    name_part = f"{student_name}, " if student_name else ""
...
• Your family, when you feel comfortable sharing
...
**Trusted Adults:**
Many young people find it helpful to talk with trusted adults who can provide ongoing support:
• School counselors or supportive teachers
• Family members when you feel ready and safe
• LGBTQ+ adult mentors or community leaders

**Your Timeline:**
Remember: There's no rush to figure everything out or to come out to anyone before you're ready. You're valid, you matter, and you deserve love and acceptance. Take your time and be gentle with yourself.
...
with st.sidebar:
    ...
    **Talk to a trusted adult**
    ...
    # ChatGPT Refinement: Development logging for missed euphemisms
    if 'potential_crisis_logs' in st.session_state and st.session_state.potential_crisis_logs:
        st.subheader("🔍 Development Monitoring")
        with st.expander("Potential Crisis Euphemisms (Dev Only)"):
            st.caption("Messages with concerning words that didn't trigger crisis detection:")
            for log in st.session_state.potential_crisis_logs[-3:]:  # Show last 3
                st.text(f"🕐 {log['timestamp'][:19]}")
                st.text(f"💬 {log['message']}")
                if log['indicators']:
                    st.text(f"🔤 Words: {', '.join(log['indicators'])}")
                if log['combinations']:
                    st.text(f"🔗 Combinations: {', '.join(log['combinations'])}")
                st.text("---")
